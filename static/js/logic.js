...
//   console.log(data)
 ...
 for (var i = 0; i < data.length; i++) {
    ...
    meanArrivalDelayMarkers.push(
      ...
      }).bindPopup("<h4>" + data[i].origin_airport_name + "</h4> <hr> <p>Mean Arrival Delay: " + data[i].mean_arrival_delay + "</p>")    //"</h3> <h3>"
    );
    ...
    flightCountMarkers.push(
      ...
      }).bindPopup("<h4>" + data[i].origin_airport_name + " (" +  data[i].origin_airport + ")" + "</h4> <hr> <p>" + data[i].origin_city + ", " +  data[i].origin_state + "</p> <hr> <p>Flight Count: " + data[i].flight_count + "</p>")
    );
    ...
    percentDelayedFlights.push(
      ...
      }).bindPopup("<h4>" + data[i].origin_airport_name + "</h4> <hr> <p>Delayed Flight Percentage: " + data[i].percent_delayed_flights + "%</p>")    //"</h3> <h3>"
    );
  }
...
d3.json(donut2Url).then(function(dataset) {
  ...
  var svg = d3.select('#donutChart') // select element in the DOM with id 'chart'
      .append('svg') // append an svg element to the element we've selected
      .attr('width', width) // set the width of the svg element we just added
      .attr('height', height) // set the height of the svg element we just added
      .append('g') // append 'g' element to the svg element
  ...
  var tooltip = d3.select('#donutChart') // select element in the DOM with id 'chart'
      .append('div') // append a div element to the element we've selected                                    
      .attr('class', 'tooltip'); // add class 'tooltip' on the divs we just selected
...
d3.json(barUrl, function (d, i, columns) {
  ...
  return d;
    ...
    // console.log("bar chart", d[columns[i]])
